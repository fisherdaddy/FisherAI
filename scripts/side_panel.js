...
function initResultPage() {
  ...
  populateModelSelections().then(async () => {
    ...
    translateButton.addEventListener('click', async function() {
      ...
      await clearAndGenerate(model, provider, translatePrompt + inputText, null, { role: 'user', text: `[网页翻译] ${currentURL}` });
    });
    ...
    videoTranslateButton.addEventListener('click', async function() {
      ...
      await clearAndGenerate(model, provider, subTitleTransPrompt + inputText, null, { role: 'user', text: `[视频翻译] ${currentURL}` });
    });
